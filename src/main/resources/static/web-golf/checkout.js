...
createApp({
    ...
    methods: {
        ...
        getDeliveryCost() {
            ...
            axios.post('/api/current/delivery-cost', `zipCode=${this.zipCode}`)
                .then(response => {
        ...
        pay() {
            ...
            axios.post('https://mindhub-brothers.up.railway.app/api/cards/pay', {
                ...
                .then(response => {
                    console.log(response)
                    axios.post('/api/current/order-purchases/generate', {orderProducts: orders})
                        .then(response => {
                            console.log(response)
                            window.location.href = '/api/current/pdf/generate'
                            localStorage.clear()
                            Swal.fire({
                                showConfirmButton: false,
                                timer:            2000,
